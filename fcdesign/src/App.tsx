...
import { Menu, X, ArrowRight, ArrowLeft, Shield, Users, Search, TrendingUp, BarChart3, Globe, ChevronRight } from 'lucide-react';
...
const faqs = [
  { q: "Что такое Финансовый центр?", a: "Это государственный интегратор финансирования программ Республики Казахстан. Мы обеспечиваем прозрачность и эффективность использования бюджетных средств." },
  { q: "Как участвовать в госпрограммах?", a: "Через портал egov.kz или напрямую на платформе Финансового центра. Для подачи заявки требуется ЭЦП." },
  { q: "Где можно посмотреть отчёты?", a: "В разделе «Отчёты и аналитика» после авторизации или в открытых данных на сайте." },
];
    ...
    <>
        ...
        <button
          ...
          className={`absolute left-6 md:left-10 top-1/2 -translate-y-1/2 z-30 backdrop-blur-md p-4 md:p-5 rounded-2xl transition-all border shadow-xl
            ${isFirst
              ...
              : 'bg-black/70 hover:bg-black/90 text-white hover:scale-110 active:scale-95 border-white/10'}`}
        >
          ...
          <div className="bg-gradient-to-r from-sky-900/30 to-zinc-900 border border-sky-500/20 rounded-3xl p-10 mb-12 flex flex-col md:flex-row items-center justify-between gap-8">
            ...
            <div className="flex gap-10 text-center">
              <div>
                ...
                <div className="text-zinc-400 text-sm mt-1">Ключевых инвесторов</div>
              </div>
              <div>
                <div className="text-3xl font-bold text-white">4</div>
                <div className="text-zinc-400 text-sm mt-1">Программы</div>
              </div>
              <div>
                <div className="text-3xl font-bold text-white">2026</div>
                <div className="text-zinc-400 text-sm mt-1">Отчётный год</div>
              </div>
            </div>
          </div>
            ...
            {investors.map((inv, i) => (
              ...
              >
                <div className="flex items-start gap-5 mb-6">
                  <div className="w-14 h-14 rounded-2xl bg-sky-900/40 border border-sky-500/20 flex items-center justify-center text-sky-400 font-bold text-lg flex-shrink-0">
                    {inv.logo}
                  </div>
                  <div>
                ...
                <div className="flex items-end justify-between">
                  <div>
                    <div className="text-2xl font-bold text-white">{inv.volume}</div>
                    <div className="text-zinc-500 text-xs mt-1">Объём вложений</div>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-bold text-sky-400">{inv.share}</div>
                    <div className="text-zinc-500 text-xs mt-1">Доля</div>
                  </div>
                </div>
                {/* Mini bar */}
      ...
      <section id="faq" className="py-24 bg-zinc-900">
        <div className="max-w-4xl mx-auto px-6">
          <h2 className="text-5xl font-bold text-center mb-16">Часто задаваемые вопросы</h2>
          <div className="space-y-6">
            {faqs.map((faq, i) => (
              <div key={i} className="bg-zinc-950 border border-zinc-800 rounded-3xl p-8">
                <h3 className="font-semibold text-xl mb-4">• {faq.q}</h3>
